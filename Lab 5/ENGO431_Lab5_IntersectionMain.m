...
% Solving an intersection for each pair of points
for i = 1:size(intersectionImageCoords,1)

    [xhatValid, rValuesVal] = performCollinearityLSA(cVal,dataright(i,:), EOP,dataleft(i,:));
    xhatVal(i, :) = xhatValid;
    rNumVal(i, :) = rValuesVal;
end

% Computing the redundancy 
rSumTrue = sum(sum(rNumVal));
...
for i = 1:size(intersectionCorrectedCoords,1)
    [xhatTrue, rValuesTrue] = performCollinearityLSA(c,datarightTrue(i,:), EOPs,dataleftTrue(i,:));
    xhat(i, :) = xhatTrue;
    rVal(i, :) = rValuesTrue';
end

% Computing the redundancy 
rSumTrue = sum(sum(rVal));

% Computing the mena height 
